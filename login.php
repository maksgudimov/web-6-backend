...
require 'connect.php';//Подключение файла connect.php, в котором создано sqli соеденение с бд
...
if(mysqli_num_rows($check_user) > 0){//Проверка на то существует ли такой логин
    ...
    if(password_verify($password,$user['pass'])){// Проверка на совпадение пароля из таблицы с введеным
        ...
        $check_power = mysqli_query($connect, "SELECT * FROM super_power1 WHERE human_id = $id");//Проверяем наличие способностей
